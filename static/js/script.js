...
(function () {
	...
	$(function () {
		...
		// Additional class on templates if mac os.
				...
				for (var i = 0; i < plugins.search.length; i++) {
					var searchItem = $(plugins.search[i]),
						options = {
							...
							template: (searchItem.attr('data-search-templates')) ? searchItem.attr('data-search-templates') : defaultTemplate,
		...
		if (plugins.selectFilter.length) {
			...
			for (i = 0; i < plugins.selectFilter.length; i++) {
				var select = $(plugins.selectFilter[i]),
					selectStyle = 'templates-' + select.attr('data-style') + '-select';
